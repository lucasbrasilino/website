...
If you worked on large Petalinux projects, you've probably figured out that it's quite cumbersome to add additional
...
optimized for your solution. But if you work on research (or R&D), and you need to experiment different libraries, tools
...
Even better, it is way easier to integrate newest softwares to your embedded system using it than the Petalinux
rootfs. There are out there some Ubuntu-based rootfs as well, but I've found these images come and go. Looks like there's
no community maintaining/updating them. **Fear not!!** The steps covered here can also be used to generate a Ubuntu rootfs,
provided that minor changes are made, like distribution name and URL.

In this post we'll explore how to create a custom rootfs based on Debian combined with the Petalinux Kernel to
...
on creating a image to be used in boards like the [KRIA K26
...
well. I'm using Ubuntu 18.04 since it is officially supported by Petalinux tools, which is necessary to compile the
Kernel.

# Setting up a SDCard

For this example we'll be using a SDCard to hold the _BOOT_ and _ROOTFS_ partitions. The boot partition is a FAT32
...
Here I will be using a 8GB SDCard, just because that's what I have on hand. Usually the generated Debian rootfs I've
...
Using `fdisk`, we'll create a 256MB partition for _BOOT_, set it as **bootable** and the use the remaining space for
_ROOTFS_:
...
Now we can format each partition. I recommend to label them, so we can use their labels to reference partitions when
...
intended to create a rootfs to be used in virtual machines. `qemu-debootstrap` actually runs the well-known
...
The following command may be used to setup a password for `root` superuser. Replace the `yoursecurepassword` password
...
Besides `ssh` to the board, you also may find useful to login using the board's serial console, which usually comes up
...
Petalinux kernel to mount the rootfs from SDCard.
...
partition (the partition we labelled as `BOOT`).
...
In this post we went through in the process of creating a custom Debian rootfs for a Zynq Ultrascale+ MPSoC board. We've
used the ZCU102, however the same process will probably work on other boards as well. With similar procedure you can
also create a image for Zynq-7000 based board, just use the `armhf` architecture, instead of `arm64`.